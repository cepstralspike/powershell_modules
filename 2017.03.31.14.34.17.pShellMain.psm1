...
function sow { # {{{
    ...
    try {
        $junk = $args[1].psobject.typenames
    }
    catch {
        $junk = $args[1]
        write-host -foregroundcolor magenta "caught sow bug"
        write-host -foregroundcolor magenta "[$junk]"
        write-host -foregroundcolor magenta "end catch"
    }
...
if ("T39247" -eq $GLOBAL:localhost) {
    $GLOBAL:sambahost = "vanguard.com\ENTDFS\HOME41\NOMAD"
}
else{
...
$GLOBAL:homeNet = "\\vanguard.com\ENTDFS\HOME41\NOMAD"
$GLOBAL:netLclDsk = "H:"
$GLOBAL:homeLclDsk = "C:"
$GLOBAL:uid = $env:USERNAME
...
if ( -not ( Test-path -path $psDiaryLclD -PathType container ) ) {
  new-item -type directory -path $psDiaryLclD
}
if ( -not ( Test-path -path "$psDiaryLclD/temp" -PathType container ) ) {
  new-item -type directory -path "$psDiaryLclD/temp"
}
